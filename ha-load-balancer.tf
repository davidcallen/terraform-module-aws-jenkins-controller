...
# ---------------------------------------------------------------------------------------------------------------------
resource "aws_acm_certificate" "jenkins-controller-amazon-provider" {
...
locals {
  https_cert_domain_validation_options = (var.route53_enabled && var.ha_high_availability_enabled && (
    (var.ha_public_load_balancer.enabled && var.ha_public_load_balancer.ssl_cert.use_amazon_provider)
    || (var.ha_private_load_balancer.enabled && var.ha_private_load_balancer.ssl_cert.use_amazon_provider))
  ) ? aws_acm_certificate.jenkins-controller-amazon-provider[0].domain_validation_options : []
}
resource "aws_route53_record" "jenkins-controller-amazon-provider-https-cert-validation" {
  for_each = {
    # for dvo in aws_acm_certificate.jenkins-controller-amazon-provider.domain_validation_options : dvo.domain_name => {
...
resource "aws_acm_certificate_validation" "jenkins-controller-amazon-provider" {
  ...
  certificate_arn         = aws_acm_certificate.jenkins-controller-amazon-provider[0].arn
...
resource "aws_lb_listener_certificate" "jenkins-controller-public" {
  count           = (var.ha_high_availability_enabled && var.ha_public_load_balancer.enabled) ? 1 : 0
...
resource "aws_lb_listener_rule" "jenkins-controller-public-https" {
  count        = (var.ha_high_availability_enabled && var.ha_public_load_balancer.enabled) ? 1 : 0
  listener_arn = var.ha_public_load_balancer.alb_listener_arn
  priority     = var.ha_public_load_balancer.alb_listener_priority
  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.jenkins-controller-public-https[count.index].arn
  }
  condition {
    host_header {
      values = [var.ha_public_load_balancer.hostname_fqdn]
    }
  }
}
locals {
...
resource "aws_lb_target_group" "jenkins-controller-public-https" {
  count                = (var.ha_high_availability_enabled && var.ha_public_load_balancer.enabled) ? 1 : 0
...
resource "aws_lb_listener_certificate" "jenkins-controller-private" {
...
# Note that the aws_lb_listener is defined outside this module and shared with others
resource "aws_lb_listener_rule" "jenkins-controller-private-https" {
  count = (var.ha_high_availability_enabled && var.ha_private_load_balancer.enabled) ? 1 : 0
  ...
  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.jenkins-controller-private-https[count.index].arn
  }
  condition {
    host_header {
      values = [var.ha_private_load_balancer.hostname_fqdn]
    }
  }
}
locals {
...
resource "aws_lb_target_group" "jenkins-controller-private-https" {
  count = (var.ha_high_availability_enabled && var.ha_private_load_balancer.enabled) ? 1 : 0
