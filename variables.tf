...
variable "disk_jenkins_home" {
  description = "Disk storage options for Jenkins Home Dir. Will be mounted at /mnt/jenkins and symlinked to /var/lib/jenkins"
  type = object({
    enabled = bool
    type    = string
    # EBS or EFS
...
variable "jenkins_linux_user_name" {
  description = "Linux user for running Jenkins process under."
  type        = string
  default     = "jenkins"
}
variable "jenkins_linux_user_group" {
  description = "Linux user group for running Jenkins process under."
  type        = string
  default     = "jenkins"
}
variable "jenkins_user_ssh_public_key" {
  description = "SSH Public Key for ssh login as Jenkins user"
  type        = string
  default     = ""
}
variable "jenkins_worker_ssh_public_key" {
  description = "SSH Public Key for ssh login as Jenkins Worker user on Jenkins Node"
  type        = string
  default     = ""
}
variable "jenkins_config_file_pathnames" {
  description = "Config file path and names to be loaded by Jenkins configuration-as-code plugin on 1st launch."
  type        = list(string)
  default     = []
}
variable "jenkins_config_files" {
  description = "Config file names and base64-encoded contents to be loaded by Jenkins configuration-as-code plugin on 1st launch."
  type = list(object({
    filename          = string
...
variable "jenkins_admin_user_password_secret_id" {
  description = "AWS Secrets ID for password to login to Jenkins application as 'admin' user."
  type        = string
  default     = ""
}
variable "jenkins_nexus_user_password_secret_id" {
  description = "AWS Secrets ID for password to login to Nexus server as 'jenkins' user. For publishing artifacts to Nexus."
  type        = string
  default     = ""
}
