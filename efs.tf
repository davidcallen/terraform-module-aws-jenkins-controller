# ---------------------------------------------------------------------------------------------------------------------
# EFS filesystem for Jenkins HOME DIR
# ---------------------------------------------------------------------------------------------------------------------
resource "aws_efs_file_system" "jenkins-home-efs" {
  count = (var.disk_jenkins_home.enabled && var.disk_jenkins_home.type == "EFS") ? 1 : 0
...
resource "aws_efs_mount_target" "jenkins-home-efs" {
  count           = (var.disk_jenkins_home.enabled && var.disk_jenkins_home.type == "EFS") ? length(var.vpc.private_subnets_ids) : 0
  file_system_id  = aws_efs_file_system.jenkins-home-efs[0].id
  subnet_id       = var.vpc.private_subnets_ids[count.index]
  security_groups = [aws_security_group.jenkins-home-efs[0].id]
}
...
resource "aws_security_group" "jenkins-home-efs" {
  count       = (var.disk_jenkins_home.enabled && var.disk_jenkins_home.type == "EFS") ? 1 : 0
  name        = "${local.name}-efs"
